<script>
  ...
  let homepageNotes = homepageNoteSlugs.map((slug) =>
    allPosts.find((note) => note.frontmatter.slug === slug)
  );
</script>

<div class="">
  <h3 class="">Try these</h3>
  <ul class="">
    {#each homepageNotes as note}
      <NoteListItem
        url={note.url}
